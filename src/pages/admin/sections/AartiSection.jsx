"use client"

import { useState, useEffect } from "react"
import { contentService } from "../../../services/api"
import Toast from "../Toast"

const AartiSection = () => {
  const [searchTerm, setSearchTerm] = useState("")
  const [aartis, setAartis] = useState([])
  const [loading, setLoading] = useState(true)
  const [showModal, setShowModal] = useState(false)
  const [editingAarti, setEditingAarti] = useState(null)
  const [toast, setToast] = useState(null)
  const [pdfPreview, setPdfPreview] = useState(null)

  const [formData, setFormData] = useState({
    name: "",
    pdfFile: null,
  })

  const showToast = (message, type) => {
    setToast({ message, type })
  }

  useEffect(() => {
    fetchAartis()
  }, [])

  const fetchAartis = async () => {
    try {
      const data = await contentService.getAartis()
      setAartis(data.aartis || [])
    } catch (error) {
      showToast("Error fetching aartis", "error")
    } finally {
      setLoading(false)
    }
  }

  const handleInputChange = (e) => {
    const { name, value } = e.target
    setFormData((prev) => ({ ...prev, [name]: value }))
  }

  const handleFileChange = (e) => {
    setFormData((prev) => ({ ...prev, pdfFile: e.target.files[0] }))
  }

  const openModal = (aarti = null) => {
    setEditingAarti(aarti)
    if (aarti) {
      setFormData({
        name: aarti.name || "",
        pdfFile: null,
      })
    } else {
      setFormData({
        name: "",
        pdfFile: null,
      })
    }
    setShowModal(true)
  }

  const closeModal = () => {
    setShowModal(false)
    setEditingAarti(null)
    setFormData({
      name: "",
      pdfFile: null,
    })
  }

  const handleSubmit = async (e) => {
    e.preventDefault()
    setLoading(true)

    try {
      const formDataToSend = new FormData()
      formDataToSend.append('name', formData.name)
      if (formData.pdfFile) {
        formDataToSend.append('pdfFile', formData.pdfFile)
      }

      if (editingAarti) {
        await contentService.updateAarti(editingAarti._id, formDataToSend)
        showToast("Aarti updated successfully!", "success")
      } else {
        await contentService.createAarti(formDataToSend)
        showToast("Aarti added successfully!", "success")
      }

      fetchAartis()
      closeModal()
    } catch (error) {
      showToast("Error saving aarti", "error")
    } finally {
      setLoading(false)
    }
  }

  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this aarti?")) {
      try {
        await contentService.deleteAarti(id)
        setAartis(aartis.filter((a) => a._id !== id))
        showToast("Aarti deleted successfully", "success")
      } catch (error) {
        showToast("Error deleting aarti", "error")
      }
    }
  }

  const filteredAartis = aartis.filter((aarti) => aarti.name?.toLowerCase().includes(searchTerm.toLowerCase()))

  if (loading && aartis.length === 0) {
    return (
      <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
        <div className="flex justify-center items-center h-64">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-500"></div>
        </div>
      </div>
    )
  }

  return (
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 md:mb-8 space-y-4 md:space-y-0">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Aarti Section</h1>
          <button
            onClick={() => openModal()}
            className="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors duration-200 font-medium"
          >
            Add Aarti
          </button>
        </div>

        {/* Search */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6 mb-6">
          <div className="w-full md:w-64">
            <input
              type="text"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Search aartis..."
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
            />
          </div>
        </div>

        {/* Aartis Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {filteredAartis.map((aarti) => (
            <div
              key={aarti._id}
              className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6 hover:shadow-md transition-shadow"
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900 truncate flex-1">{aarti.name}</h3>
                <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-orange-100 text-orange-800 ml-2">
                  PDF
                </span>
              </div>

              <div className="space-y-2 mb-4 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500">Created:</span>
                  <span className="text-gray-900">{aarti.createdAt?.toDate?.()?.toLocaleDateString() || "N/A"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Status:</span>
                  <span className="text-green-600 font-medium">{aarti.status}</span>
                </div>
              </div>

              <div className="flex space-x-2">
                <button
                  onClick={() => openModal(aarti)}
                  className="flex-1 px-3 py-2 text-sm bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors duration-200 font-medium"
                >
                  Edit
                </button>
                {aarti.pdfUrl && (
                  <button
                    onClick={() => setPdfPreview(aarti)}
                    className="flex-1 px-3 py-2 text-sm bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors duration-200 font-medium"
                  >
                    Preview
                  </button>
                )}
                <button
                  onClick={() => handleDelete(aarti._id)}
                  className="flex-1 px-3 py-2 text-sm bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors duration-200 font-medium"
                >
                  Delete
                </button>
              </div>
            </div>
          ))}
        </div>

        {filteredAartis.length === 0 && (
          <div className="text-center py-12">
            <div className="text-gray-500 text-lg">No aartis found</div>
          </div>
        )}

        {/* Add/Edit Modal */}
        {showModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
              <h2 className="text-xl font-semibold text-gray-900 mb-6">
                {editingAarti ? "Edit Aarti" : "Add New Aarti"}
              </h2>

              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Name of Aarti *</label>
                  <input
                    type="text"
                    name="name"
                    value={formData.name}
                    onChange={handleInputChange}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
                    required
                  />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">PDF Upload</label>
                  <input
                    type="file"
                    name="pdfFile"
                    onChange={handleFileChange}
                    accept=".pdf"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100"
                  />
                  {editingAarti && editingAarti.pdfUrl && (
                    <p className="text-sm text-gray-500 mt-1">Current PDF uploaded</p>
                  )}
                </div>

                <div className="flex space-x-4 pt-4">
                  <button
                    type="button"
                    onClick={closeModal}
                    className="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors duration-200 font-medium"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors duration-200 disabled:opacity-50 font-medium"
                  >
                    {loading ? "Saving..." : "Save"}
                  </button>
                </div>
              </form>
            </div>
          </div>
        )}

        {/* PDF Preview Modal */}
        {pdfPreview && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-hidden">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold text-gray-900">{pdfPreview.name} - PDF Preview</h2>
                <button onClick={() => setPdfPreview(null)} className="text-gray-400 hover:text-gray-600">
                  ✕
                </button>
              </div>
              <div className="h-[70vh]">
                <iframe src={pdfPreview.pdfUrl} className="w-full h-full border rounded-lg" title="PDF Preview" />
              </div>
            </div>
          </div>
        )}

        {toast && <Toast message={toast.message} type={toast.type} onClose={() => setToast(null)} />}
      </div>
    </div>
  )
}

export default AartiSection
